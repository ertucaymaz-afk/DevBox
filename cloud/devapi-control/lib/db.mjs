...
export async function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    await query`ALTER TABLE devbox_commands ADD COLUMN IF NOT EXISTS apply_status TEXT NOT NULL DEFAULT 'PENDING'`;
    await query`ALTER TABLE devbox_commands ADD COLUMN IF NOT EXISTS apply_detail TEXT`;
    await query`ALTER TABLE devbox_commands ADD COLUMN IF NOT EXISTS applied_at TIMESTAMPTZ`;
    await query`ALTER TABLE devbox_commands ADD COLUMN IF NOT EXISTS applied_instance_id TEXT`;
    await query`CREATE INDEX IF NOT EXISTS devbox_commands_project_sequence_idx ON devbox_commands(project_id, sequence ASC)`;
    await query`CREATE INDEX IF NOT EXISTS devbox_commands_project_status_idx ON devbox_commands(project_id, apply_status, sequence ASC)`;
  })();
...
async function pruneCommands(projectId) {
  ...
  await query.query(
    `DELETE FROM devbox_commands WHERE sequence IN (
      SELECT sequence FROM devbox_commands
...
export async function getProjectState(projectId) {
  ...
  const [current, history, recentCommands] = await Promise.all([
    ...
    query`SELECT sequence, id, kind, payload, apply_status, apply_detail, applied_at, applied_instance_id, created_at FROM devbox_commands WHERE project_id=${projectId} ORDER BY sequence DESC LIMIT 120`
  ]);
...
export async function insertCommand({ id, projectId, kind, payload }) {
  ...
  const rows = await query`INSERT INTO devbox_commands(id, project_id, kind, payload) VALUES (${id}, ${projectId}, ${kind}, ${json}::jsonb) RETURNING sequence, id, project_id AS "projectId", kind, payload, apply_status AS "applyStatus", created_at AS "createdAt"`;
  return rows[0];
}
