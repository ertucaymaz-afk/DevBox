...
function sanitizeSnapshot(row) {
  ...
  return {
    ...
    evolution: {
      ...
      isRunning: Boolean(evolution.isRunning),
      score: number(evolution.score),
      level: count(evolution.level),
      lifetimeLevel: count(evolution.lifetimeLevel),
      lifetimeEvidencePoints: count(evolution.lifetimeEvidencePoints),
      validatedImprovementCount: count(evolution.validatedImprovementCount),
      stablePromotionCount: count(evolution.stablePromotionCount),
      verifiedResearchCount: count(evolution.verifiedResearchCount),
      verifiedRegressionFixCount: count(evolution.verifiedRegressionFixCount),
      stage: text(evolution.stage, 160),
      spec: {
        phaseCount: count(spec.phaseCount),
        totalTaskCount: count(spec.totalTaskCount),
        passCount: count(spec.passCount),
        failedCount: count(spec.failedCount),
        blockedCount: count(spec.blockedCount),
        recoveryCount: count(spec.recoveryCount),
        remainingCount: count(spec.remainingCount),
        currentGateState: text(spec.currentGateState, 80)
      }
    },
    findings: {
      ...
      rejected: count(findings.rejected),
      blocking: count(findings.blocking),
      bySeverity: {
    ...
    releaseGate: Object.keys(gate).length ? {
      state: text(gate.state, 16),
      mode: text(gate.mode, 16),
      blockingFailures: count(gate.blockingFailures),
      completedAt: text(gate.completedAt, 80)
    } : null,
...
function send(res, status, body, etag = null) {
  ...
  res.setHeader("x-devbox-public-state", "sanitized");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return send(res, code.includes("UNCONFIGURED") ? 503 : 500, {
      error: code.includes("UNCONFIGURED") ? "CONTROL_PLANE_UNCONFIGURED" : "PUBLIC_STATE_FAILED",
      version: VERSION
    });
  }
}
