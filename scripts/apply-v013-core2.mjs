...
async function evolution() {
  ...
  s = once(s, `  readonly #runner: CommandRunner;
  readonly #inFlight`, `  readonly #runner: CommandRunner;
...
`      let baselineStatus = await this.#git.status(executionRoot);
  ...
  s = s.split('await this.#restoreManagedWorkspace(project.rootPath, baselineHead)').join('await this.#restoreManagedWorkspace(executionRoot, baselineHead)');
  s = s.split('this.#verifyWorkspace(project.rootPath, baselineFingerprint, controller.signal)').join('this.#verifyWorkspace(executionRoot, baselineFingerprint, controller.signal)');
  s = s.split('this.#commitVerifiedMutation(project.rootPath, specTask, controller.signal)').join('this.#commitVerifiedMutation(executionRoot, specTask, controller.signal)');
  s = once(s,
...
`      const recovery = !cancelled && (rollbackFailed || (!isolatedEvolutionRoot && (message.includes("EVOLUTION_WORKSPACE_DIRTY_BASELINE") || message.includes("EVOLUTION_BASELINE_HEAD_INVALID"))));`, "continuous-retry");
  ...
  const anchor = `  async #commitVerifiedMutation(rootPath: string, specTask: DevelopmentSpecTask, cancellation: AbortSignal): Promise<string[]> {`;
  const method = `  async #commitEvidenceSnapshot(rootPath: string, specTask: DevelopmentSpecTask, cancellation: AbortSignal): Promise<string[]> {
...
`;
  s = once(s, anchor, method + anchor, "evidence-commit-method");
  await save(file, s);
}
...
`, "app-target-infer");
  ...
  s = once(s, `  const [workspaceResult, setWorkspaceResult] = useState<ThreadWorkspaceResult | null>(null);
  const [changeSummaryOpen`, `  const [workspaceResult, setWorkspaceResult] = useState<ThreadWorkspaceResult | null>(null);
  ...
  s = once(s, `    setWorkspaceResult(null);
    setView("thread");`, `    setWorkspaceResult(null);
    setLiveWorkspacePath(null);
    setLiveWorkspaceActive(false);
    setView("thread");`, "app-new-thread-reset");
  s = once(s, `    setComposer("");
    setWorkspaceResult(null);
    setChangeSummaryOpen(false);`, `    const liveTarget = inferWorkspaceTargetPath(content);
    setComposer("");
    setWorkspaceResult(null);
  ...
  s = once(s, `  useEffect(() => window.devbox.onThreadWorkspaceResult((result) => {
    setWorkspaceResult(result);
    setInspectorVisible(true);`, `  useEffect(() => window.devbox.onThreadWorkspaceResult((result) => {
    setWorkspaceResult(result);
    setLiveWorkspaceActive(false);
    setLiveWorkspacePath(result.previewPath ?? result.primaryFile ?? null);
    setInspectorVisible(true);`, "app-workspace-result-live");
  s = once(s, `                    {busy === "message" && liveActivities.filter((activity) => activity.threadId === thread.thread.id).map((activity, index) => <LiveActivity key={\`\${activity.createdAt}:\${index}\`} event={activity} />)}
                    {busy === "message" && !liveActivities.some((activity) => activity.threadId === thread.thread.id) && <div className="activity-line running"><LoaderCircle className="spin" size={14} /><span>İstek izin ve ek bağlam kontrollerinden geçiyor…</span></div>}`, `                    {busy === "message" && <div className="activity-line running compact"><LoaderCircle className="spin" size={14} /><span>{liveWorkspaceActive ? "Gerçek dosya değişiklikleri diske yazılıyor ve Canvas kod görünümü canlı okunuyor…" : "DevBox yanıt hazırlıyor…"}</span></div>}`, "app-compact-progress");
  s = once(s, `          {inspectorVisible && <CanvasInspector project={selectedProject} result={workspaceResult} threadTitle=`, `          {inspectorVisible && <CanvasInspector project={selectedProject} result={workspaceResult} liveTargetPath={liveWorkspacePath} liveActive={liveWorkspaceActive} threadTitle=`, "app-canvas-live-props");
  await save(file, s);
}

async function canvas() {
  const file = "src/renderer/CanvasInspector.tsx"; let s = await read(file);
  s = once(s, `  result: ThreadWorkspaceResult | null;
  threadTitle: string | null;`, `  result: ThreadWorkspaceResult | null;
  liveTargetPath: string | null;
  liveActive: boolean;
  threadTitle: string | null;`, "canvas-live-props-type");
  s = once(s, `  const { project, result, threadTitle, threadState, gitBranch, coreState, onClose, onRefresh } = props;`, `  const { project, result, liveTargetPath, liveActive, threadTitle, threadState, gitBranch, coreState, onClose, onRefresh } = props;`, "canvas-live-props");
  s = once(s, `  const [consoleEntries, setConsoleEntries] = useState<ConsoleEntry[]>([]);
  const iframeRef`, `  const [consoleEntries, setConsoleEntries] = useState<ConsoleEntry[]>([]);
  const [previewState, setPreviewState] = useState<"idle" | "loading" | "ready" | "error">("idle");
  const iframeRef`, "canvas-preview-state");
  s = once(s, `  const defaultTargetPath = result?.previewPath ?? result?.primaryFile ?? null;`, `  const defaultTargetPath = liveTargetPath ?? result?.previewPath ?? result?.primaryFile ?? null;`, "canvas-live-target");
  s = once(s, `    } catch (error) {
      setSnapshot(null); setCode(""); setNotice(error instanceof Error ? error.message : String(error));
    } finally { setBusy(false); }`, `    } catch (error) {
  ...
  s = once(s, `  useEffect(() => { setSelectedPath(defaultTargetPath); }, [defaultTargetPath, result?.createdAt]);
  ...
  }, [result?.createdAt, result?.previewPath, result?.changedFiles.length]);`, `  useEffect(() => { setSelectedPath(defaultTargetPath); }, [defaultTargetPath, result?.createdAt]);
  useEffect(() => { void loadFile(); }, [loadFile, result?.createdAt]);
  useEffect(() => {
  ...
  s = once(s, `      if (payload.type === "console" && typeof payload.message === "string") {`, `      if (payload.type === "ready") { setPreviewState("ready"); return; }
      if (payload.type === "console" && typeof payload.message === "string") {
        if (payload.level === "error") setPreviewState("error");`, "canvas-ready-event");
  s = s.replace(`<strong>{targetPath ?? "Dosya yok"}</strong><small>{snapshot ? \`\${snapshot.language} · SHA \${snapshot.sha256.slice(0, 10)}\` : "yüklenmedi"}</small>`, `<strong>{targetPath ?? "Dosya yok"}</strong><small>{liveActive ? (snapshot ? "CANLI · diskten gerçek dosya okunuyor" : "CANLI · ilk dosya yazımı bekleniyor") : snapshot ? \`\${snapshot.language} · SHA \${snapshot.sha256.slice(0, 10)}\` : "yüklenmedi"}</small>`);
  s = s.replace(`<strong>{result?.previewPath ?? "HTML önizleme yok"}</strong><small>İzole yerel önizleme</small>`, `<strong>{result?.previewPath ?? "HTML önizleme yok"}</strong><small>İzole yerel önizleme · {previewState === "ready" ? "HAZIR" : previewState === "error" ? "HATA" : "YÜKLENİYOR"}</small>`);
  s = s.replace(`<iframe ref={iframeRef} title={\`Önizleme: \${result.previewPath}\`} sandbox="allow-scripts allow-same-origin" src={previewUrl(project.id, result.previewPath, revision)} />`, `<iframe ref={iframeRef} title={\`Önizleme: \${result.previewPath}\`} sandbox="allow-scripts allow-same-origin" onLoad={() => setPreviewState((current) => current === "error" ? current : "loading")} onError={() => setPreviewState("error")} src={previewUrl(project.id, result.previewPath, revision)} />`);
  await save(file, s);
}

await previewProtocol();
await worktree();
await evolution();
await main();
await ipc();
