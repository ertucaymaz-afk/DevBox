...
if (start < 0 || end < 0 || end <= start) throw new Error("V014_CORE5_SCHEMA7_BLOCK_MISSING");
let block = source.slice(start, end);
const signature = "  public integrityCheck()";
const occurrences = block.split(signature).length - 1;
if (occurrences !== 2) throw new Error(`V014_CORE5_SCHEMA7_SIGNATURE_COUNT:${occurrences}`);
block = block.replaceAll(signature, "  public integrityCheck(): { ok: boolean; detail: string; schemaVersion: number } {");
source = source.slice(0, start) + block + source.slice(end);
const temporary = path.resolve("scripts/.apply-v014-core5-runtime.mjs");
