...
try{
 for(const route of routes)for(const viewport of viewports){
  const context=await browser.newContext({viewport:{width:viewport.width,height:viewport.height},reducedMotion:"reduce",colorScheme:"dark"});const page=await context.newPage();const consoleErrors=[],pageErrors=[];
  ...
  const slug=route==="/"?"home":route.slice(1),dir=path.join(outputRoot,slug);fs.mkdirSync(dir,{recursive:true});const screenshot=path.join(dir,`${viewport.name}.png`);await page.screenshot({path:screenshot,fullPage:true,animations:"disabled"});const bytes=fs.statSync(screenshot).size;if(bytes<2000)throw Error(`WEB_VISUAL_SCREENSHOT_TOO_SMALL:${route}:${viewport.name}:${bytes}`);
  results.push({route,viewport:viewport.name,width:viewport.width,height:viewport.height,httpStatus:response.status(),browserChannel:"chrome",reducedMotion:true,horizontalOverflowPx:overflow,commandPalette:"PASS",truthFixture:"STALE",architectureExplorer:route==="/"?"PASS":"N/A",sourceCapabilityMatrix:route==="/"?"12/12":"N/A",evolutionTracks:route==="/"?"10/10":"N/A",consoleErrors:0,pageErrors:0,screenshot:path.relative(root,screenshot).replaceAll("\\","/"),screenshotBytes:bytes});await context.close();
 }
}catch(error){failure=error}finally{await browser.close();await new Promise(resolve=>server.close(resolve))}
const summary={schemaVersion:3,product:"DevBox web ecosystem",sourceSha:process.env.GITHUB_SHA||null,generatedAt:new Date().toISOString(),fixturePolicy:"QA_ONLY_STALE_PUBLIC_STATE_NO_PRODUCTION_CLAIM",browser:"RUNNER_PROVIDED_GOOGLE_CHROME",routes:routes.length,viewports:viewports.length,expectedScreenshots:44,capturedScreenshots:results.length,architectureExplorer:"BROWSER_VERIFIED_ON_HOME",sourceCapabilities:12,evolutionTracks:10,reducedMotion:"VERIFIED_PER_CASE",status:failure?"FAIL":"PASS",error:failure instanceof Error?failure.message:failure?String(failure):null,results};fs.writeFileSync(path.join(outputRoot,"summary.json"),`${JSON.stringify(summary,null,2)}\n`);
if(failure)throw failure;if(results.length!==44)throw Error(`WEB_VISUAL_MATRIX_INCOMPLETE:${results.length}`);console.log("DEVBOX_WEB_VISUAL_VERIFY_PASS routes=11 viewports=4 screenshots=44 browser=system-chrome architecture=10 capabilities=12 evolutionTracks=10 reducedMotion=verified truthFixture=STALE");
