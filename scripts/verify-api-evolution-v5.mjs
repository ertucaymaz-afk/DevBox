...
function check(name, condition, detail = "") {
  ...
  if (!condition) throw new Error(`API_EVOLUTION_V5_VERIFY_FAILED:${name}${detail ? `:${detail}` : ""}`);
}

const [contracts, service, agent, capability, specService, database, runner, renderer, styles, bridge, preload, ipc, builder, signedBuilder, main, mcpHost, localCatalog, sourceSync, graphText, researchText] = await Promise.all([
  ...
  read("config/electron-builder.signed.cjs"), read("src/main/main.ts"), read("src/main/services/mcp-host-service.ts"), read("src/main/services/local-catalog-service.ts"), read("scripts/sync-source-archive.ps1"),
...
check("packaged-spec-resource", builder.includes("geliştirme-spec-task-graph.json") && builder.includes("development/geliştirme-spec-task-graph.json") && builder.includes("specs/development/geliştirme.md") && builder.includes("development/geliştirme.md"));
check("signed-packaged-spec-resource", signedBuilder.includes("geliştirme-spec-task-graph.json") && signedBuilder.includes("specs/development/geliştirme.md"));
check("runtime-spec-source-integrity", main.includes("developmentSpecSourcePath") && specService.includes("DEVELOPMENT_SPEC_SOURCE_SHA_MISMATCH") && specService.includes("DEVELOPMENT_SPEC_SOURCE_LINES_MISMATCH"));
...
check("source-archive-includes-spec-source", sourceSync.includes('specs/development/geliştirme.md'));
...
console.log(`API_EVOLUTION_V5_VERIFY_PASS checks=${checks.length} tasks=${graph.tasks.length} phases=${graph.summary.phaseCount}`);
