...
if (evidence.vercel?.devbox?.projectId == null) evidence.vercel.devbox.canonicalUrl = null;
if (evidence.release?.productionEvidence !== "PASS" && evidence.canary?.crossSiteLinks === "PASS") evidence.canary.crossSiteLinks = "PENDING";
await writeFile(evidenceFile, `${JSON.stringify(evidence, null, 2)}\n`, "utf8");

const links = JSON.parse(await readFile("cloud/product-links.json", "utf8"));
if (links.devbox?.canonicalUrl && !evidence.vercel?.devbox?.projectId) throw new Error("V020_UNVERIFIED_DEVBOX_CANONICAL_URL");
if (links.devapi?.canonicalUrl !== evidence.vercel?.devapi?.canonicalUrl) throw new Error("V020_DEVAPI_CANONICAL_EVIDENCE_DRIFT");
