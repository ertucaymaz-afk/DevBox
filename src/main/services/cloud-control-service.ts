...
export class CloudControlService {
  ...
  async #request(projectId: string, pathname: string, init: { method: "GET" | "POST"; body?: unknown }): Promise<unknown> {
  ...
  public async poll(projectId: string): Promise<CloudControlStatus> {
    ...
    try {
      ...
      for (const candidate of commands.slice(0, 100)) {
        ...
        if (typeof value.id !== "string" || typeof value.sequence !== "number" || value.projectId !== projectId || !["evolution.setEnabled", "evolution.run", "evolution.cancel"].includes(String(value.kind))) continue;
        ...
        const idempotencyKey = `cloud-command:${command.id}`;
        if (!this.#database.getSetting<boolean>(idempotencyKey)) {
          await this.#applyCommand(command);
          this.#database.setSetting(idempotencyKey, true);
      ...
      return this.#save(projectId, { state: "READY", pendingCommandCursor: cursor, lastCommandAt, lastError: null });
    } catch (error) {
      return this.#save(projectId, { state: "DEGRADED", lastError: boundedError(error) });
    }
  }
